...
import type { Question, Section, Template } from "@/types/template"
import type { GeoValue, MultiMeasurementValue, QuestionValue } from "@/types/response"

// A question is "answered" if its value satisfies the type's shape.
// Narrows `v` against the question type where needed — the union
// doesn't carry a tag, so we rely on Array.isArray / 'in' guards.
const isAnswered = (v: QuestionValue | undefined, q?: Question): boolean => {
...
export default function SurveyForm({ survey, template, onResponsesChange, onSubmitted }: { survey: SiteSurvey; template: Template; onResponsesChange?: (r: Record<string, QuestionValue>) => void; onSubmitted?: (survey: SiteSurvey) => void }) {
  const [responses,     setResponses] = useState<Record<string, QuestionValue>>((survey.responses as Record<string, QuestionValue>) || {})
  ...
  const triggeredFollowUps = useMemo(() => {
    ...
    template.sections.forEach((sec) => sec.questions.forEach((q) => {
      if (q.type === "multi_select" && q.options) {
        const sel = (responses[q.id] as string[] | undefined) ?? []
        q.options.forEach((opt) => { if (opt.followUpQuestionId && sel.includes(opt.id)) ids.add(opt.followUpQuestionId) })
      }
  ...
  const setAnswer = useCallback((qId: string, value: QuestionValue | null) => {
    if (isLocked) return
    setResponses(prev => {
      if (value === null) {
          ...
          <div style={{ maxWidth: 680, display: "flex", flexDirection: "column", gap: 28 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 12, borderTop: "1px solid #f1f5f9" }}>
              {isLocked ? (
                ...
                (() => {
                  const idx  = template.sections.findIndex((s) => s.id === activeSection)
