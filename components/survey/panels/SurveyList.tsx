import { STATUS_CFG, STATUSES } from '../common/shared';
...
export default function SurveyList({
  ...
  surveys: SiteSurvey[] | null;
  selected: SiteSurvey | null;
  search: string;
  statusFilter: string;
  onSearch: (v: string) => void;
  onStatusFilter: (v: string) => void;
  onSelect: (s: SiteSurvey) => void;
}) {
  const filtered =
    surveys === null
      ? []
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        style={{ padding: '16px 14px 12px', borderBottom: '1px solid #f1f5f9' }}
      >
      ...
      <div style={{ padding: '8px 14px', borderTop: '1px solid #f1f5f9' }}>
        <p style={{ fontSize: 11, color: '#94a3b8', margin: 0 }}>
          {filtered.length} of {(surveys || []).length} survey
          {(surveys || []).length !== 1 ? 's' : ''}
        </p>
      </div>
    </div>
