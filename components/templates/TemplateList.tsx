...
export default function TemplateList({ groups, selectedId, search, onSearch, onSelect, onNew, onDeleted }: {
  groups:     any[]
  ...
  return (
    <div style={{ width: 300, flexShrink: 0, borderRight: '1px solid #e2e8f0', background: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {filtered.map(g => (
          <TemplateListItem key={g.groupId} group={g} selectedId={selectedId} onSelect={onSelect} onDeleted={onDeleted} />
        ))}
      </div>
